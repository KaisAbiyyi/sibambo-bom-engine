<script lang="ts">
  ...
  function stagePalette() {
    return theme === "dark"
      ? { background: "#141310", ground: "#202018" }
      : { background: "#efe4cf", ground: "#ded0b6" };
  }
  ...
  function materialForState(layer: LayerModel, visualState: LayerVisualState) {
    const opacity = visualState === "ghost" ? 0.18 : visualState === "muted" ? 0.62 : 0.95;
    const color =
      visualState === "active"
        ? layer.color
        : theme === "dark"
  ...
  function buildSceneModel(nextModel: ParsedModel | null) {
    ...
    Object.entries(nextModel.layers).forEach(([key, layer]) => {
      ...
      const edgeMaterial = new LineBasicMaterial({
        color: visualState === "active" ? "#292724" : "#8f8577",
        transparent: true,
        opacity: visualState === "active" ? 0.68 : 0.22
      });
    ...
    layerRuntimes.forEach((runtime, key) => {
      ...
      if (runtime.visualState !== visualState || runtime.materialTheme !== theme) {
        ...
        runtime.edgeMaterial = new LineBasicMaterial({
          color: visualState === "active" ? (theme === "dark" ? "#f0e5d0" : "#292724") : theme === "dark" ? "#6f675a" : "#8f8577",
          transparent: true,
          opacity: visualState === "active" ? 0.68 : 0.22
        });
  ...
  function updateGround(nextModel: ParsedModel | null) {
    ...
    ground.scale.setScalar(Math.max(fit * 0.38, 8));
  ...
  onMount(() => {
    ...
    scene3d.add(new AmbientLight("#efe4cf", 1.8));
    const sun = new DirectionalLight("#fff4df", 2.4);
    sun.position.set(7, 11, 5);
    scene3d.add(sun);
    const fill = new DirectionalLight("#8fb5c4", 1.1);
    ...
    ground = new Mesh(
      ...
      new MeshStandardMaterial({ color: stagePalette().ground, roughness: 0.92, metalness: 0 })
    );
