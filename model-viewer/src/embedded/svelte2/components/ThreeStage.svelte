<script lang="ts">
  ...
  function stagePalette() {
    return theme === "dark"
      ? { background: "#f5f2ea", ground: "#ffffff" }
      : { background: "#f7f4ec", ground: "#ffffff" };
  }
  ...
  function materialForState(layer: LayerModel, visualState: LayerVisualState) {
    const opacity = visualState === "ghost" ? 0.22 : 1;
    const color =
      visualState === "active"
        ? "#f8f8f4"
        : visualState === "ghost"
  ...
  onMount(() => {
    ...
    scene3d.add(new AmbientLight("#ffffff", 1.35));
    const sun = new DirectionalLight("#fff8ea", 3.4);
    ...
    scene3d.add(sun);
    const fill = new DirectionalLight("#dbeafe", 1.45);
