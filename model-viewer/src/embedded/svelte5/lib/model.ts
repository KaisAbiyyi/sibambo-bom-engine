...
function finiteCoordinate(value: unknown): number {
  ...
  if (!Number.isFinite(numberValue) || Math.abs(numberValue) > MAX_COORDINATE_ABS) {
...
function addFace(
  ...
  if (entity.vertices && entity.vertices.length > MAX_VERTICES_PER_FACE) throw new Error("Face has too many vertices.");
  const vertices = entity.vertices
    ?.filter((vertex) => vertex.position)
    .map((vertex) => [finiteCoordinate(vertex.position.x), finiteCoordinate(vertex.position.z), -finiteCoordinate(vertex.position.y)] as const);
  ...
  for (let i = 1; i < vertices.length - 1; i += 1) {
    ...
    for (const vertex of tri) {
      ...
      pushBounds(layer.bounds, vertex[0], vertex[1], vertex[2]);
    }
...
export function parseBomModel(json: BomModelJson, sourceName = "BOM JSON"): ParsedModel {
  ...
  const parsedLayers: LayerStat[] = [...layers.values()]
    ...
    .map((layer) => ({
      ...
      bounds: makeBounds(layer.bounds),
  ...
  const macroMap = new Map<MacroKey, MacroStat & { rawBounds: { min: number[]; max: number[] } }>();
  for (const layer of parsedLayers) {
    const current =
      ...
      ({
        ...
        meshCount: 0,
        areaM2: 0
      } satisfies MacroStat & { rawBounds: { min: number[]; max: number[] } });
    ...
    pushBounds(current.rawBounds, layer.bounds.min[0], layer.bounds.min[1], layer.bounds.min[2]);
    pushBounds(current.rawBounds, layer.bounds.max[0], layer.bounds.max[1], layer.bounds.max[2]);
    current.bounds = makeBounds(current.rawBounds);
  ...
  return {
    ...
    layers: parsedLayers,
    macros: [...macroMap.values()].map(({ rawBounds: _rawBounds, ...macro }) => macro).sort((a, b) => b.areaM2 - a.areaM2),
