...
interface RegistryEntry {
  ...
  meshes: THREE.Mesh[];
  group: THREE.Group;
  storyState?: "active" | "context" | "ghost" | "hidden";
  storyEdges?: boolean;
...
export class ThreeViewerEngine {
  ...
  setPresentationMode(mode: "dark" | "paper") {
    ...
    if (mode === "paper") {
      ...
      const divisions = this.gridHelper.userData?.divisions || 20;
      const size = this.gridHelper.userData?.size || 20;
      this.gridHelper = new THREE.GridHelper(size, divisions, 0x263241, 0x1f2937);
      this.scene.add(this.gridHelper);
      (this.groundMesh.material as THREE.MeshStandardMaterial).color.setHex(0x0e1319);
    }
  ...
  applyStoryFocus(activeKeys: string[], contextKeys?: string[], options: StoryFocusOptions = {}) {
    ...
    this.devLog("story focus", { active: activeKeys, context: contextKeys ?? [], suppressed: options.suppressKeys ?? [] });
    
    Object.entries(this.layerRegistry).forEach(([key, entry]) => {
      let mode: "active" | "context" | "ghost" | "hidden" = "ghost";
      if (hidden.has(key)) mode = "hidden";
      else if (show.has(key)) mode = "active";
      else if (context.has(key)) mode = "context";

      const edgeState = mode === "active" ? activeEdges : false;
      if (entry.storyState === mode && entry.storyEdges === edgeState) return;
      entry.storyState = mode;
      entry.storyEdges = edgeState;
      entry.group.visible = mode !== "hidden";

      if (mode === "hidden") {
        this.hideLayerMeshes(entry);
        return;
      }
      ...
      entry.meshes.forEach((mesh) => {
        ...
        switch (mode as string) {
          case "active":
            ...
            if (this.presentationMode === "paper") {
              this.restoreActiveMaterial(mesh);
              if (edges) edges.visible = activeEdges;
            } else {
          ...
          case "context":
            mesh.visible = true;
            mesh.renderOrder = 1;
            this.assignMaterial(mesh, this.storyMaterials.context);
            if (edges) edges.visible = false;
            break;
          case "ghost":
            mesh.visible = false;
  ...
  private setupLights() {
    this.scene.add(new THREE.HemisphereLight(0xffffff, 0x334155, 0.65));
